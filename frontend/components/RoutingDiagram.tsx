...
import { ArrowRight, ArrowDown, CreditCard, Landmark, Coins, ShieldCheck, RefreshCw } from 'lucide-react';
...
export const RoutingDiagram: React.FC<RoutingDiagramProps> = ({
  ...
  const getInstrumentIcon = (assetClass?: string) => {
    switch (assetClass) {
      ...
      case 'fiat_ach':
        return <Landmark className="w-4 h-4 text-blue-600 shrink-0" />;
      default:
  ...
  return (
    <div className="bg-slate-50/70 border border-slate-200/90 rounded-xl p-4 sm:p-5 my-4">
      {/* Header bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 sm:pb-4 mb-4 border-b border-slate-200 gap-2">
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-blue-600" />
          <span className="text-xs font-bold uppercase tracking-wider text-slate-700">
            Deterministic Closed-Loop Routing Flow
          </span>
        </div>
        <div className="flex items-center gap-3 text-xs flex-wrap">
          <span className="text-slate-500">Gross Route:</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-11 gap-4 items-center">
        ...
        <div className="md:col-span-4 space-y-2.5">
          <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider flex items-center justify-between">
            <span>1. Original Deposits (FIFO)</span>
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="font-medium text-slate-800 truncate">{dep.payment_method_label}</div>
        ...
        <div className="md:col-span-3 flex flex-col items-center justify-center py-2 md:py-4 px-2">
          {/* Mobile connector: down arrow */}
          <div className="flex md:hidden items-center justify-center py-2 text-blue-600">
            <ArrowDown className="w-4 h-4" />
          </div>

          {/* Desktop connector: left-to-center */}
          <div className="w-full hidden md:flex items-center justify-center gap-2 text-slate-400 mb-2">
            <div className="h-[1px] flex-1 bg-slate-300" />
            <ArrowRight className="w-4 h-4 text-blue-600" />
            <div className="h-[1px] flex-1 bg-slate-300" />
          </div>

          <div className="bg-slate-900 text-white p-4 rounded-xl shadow-md text-center w-full border border-slate-800">
            <div className="w-8 h-8 rounded-lg bg-blue-600 mx-auto flex items-center justify-center mb-1.5 shadow-xs">
              <RefreshCw className="w-4 h-4 text-white" />
            </div>
            <div className="font-bold text-xs tracking-wider uppercase text-white">SOAP ROUTER</div>
            <div className="text-[10px] text-blue-300 font-medium">Closed-Loop Engine</div>
            <div className="mt-2 pt-2 border-t border-slate-800 text-[10px] text-slate-300 flex items-center justify-center gap-1">
          ...
          {/* Desktop connector: center-to-right */}
          <div className="w-full hidden md:flex items-center justify-center gap-2 text-slate-400 mt-2">
            <div className="h-[1px] flex-1 bg-slate-300" />
            <ArrowRight className="w-4 h-4 text-blue-600" />
            <div className="h-[1px] flex-1 bg-slate-300" />
          </div>
          ...
          {legs.map((leg, idx) => (
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="font-medium text-slate-900 truncate">{leg.payment_method_label}</div>
